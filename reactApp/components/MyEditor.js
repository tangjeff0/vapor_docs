...
import {Button, Icon, Row, Input, Modal, Collection, CollectionItem} from 'react-materialize';
...
function searchNodes(node, searchTerm, nodeArray) {
  if(node.hasChildNodes()) {
    ...
    if(node.textContent.indexOf(searchTerm) > -1){
      ...
      nodeArray.push({
        node,
        index: node.textContent.indexOf(searchTerm),
    ...
    return (
      <div className="wrapper" style={{width: '95%'}}>
        ...
        <Row className='bottom-row'>
          <Outline currentContent={this.state.editorState.getCurrentContent()} />
          <Modal
